...
class BaseParser:
    ...
    def _get_table(self, html, class_name=None, table_id=None):
        ...
        if not table:
            logger.error(f"""{self.name}. Таблица с классом '{
                         class_name}' и ID '{table_id}' не найдена.""")
        return table
    ...
    def extract_table_data(self, table, key_tag='td', min_cells=2, exact_cells=None):
        ...
        for row in rows:
            ...
            if len(cells) >= min_cells:
                ...
                value = cells[1].get_text(strip=True) if len(
                    cells) > 1 else None  # Второй элемент — значение
                if key and value:
